...
\begin{algorithm}
    \caption{Compute the Top-K Most Probable Assignments in the Graphical Model}
    \begin{algorithmic}[1]
    \State $junction_tree \gets$ getJunctionTree()
    \State $junction\_tree\_adj\_list \gets {}$
    \For{each edge $(a, b)$ in $junction\_tree$}
    \State $junction\_tree\_adj\_list[a] \gets junction\_tree\_adj\_list[a] \cup {b}$
    \State $junction\_tree\_adj\_list[b] \gets junction\_tree\_adj\_list[b] \cup {a}$
    \EndFor
    \State $root \gets$ select a maximal clique as root
    \State $depth\_map[root] \gets 0$
    \Procedure{DFS}{$node, parent, depth$}
    \For{each $child$ in $junction\_tree\_adj\_list[node]$}
    \If{$child \neq parent$}
    \State $depth\_map[child] \gets depth$
    \State \Call{DFS}{$child, node, depth + 1$}
    \EndIf
    \EndFor
    \EndProcedure
    \State \Call{DFS}{$root, \text{None}, 1$}
    \Procedure{SendMessage}{$from\_clique, to\_clique, parent\_map, clique\_potentials, messages$}
        \State $variables\_seen \gets$ all variables in $from\_clique$
        \For{each $neighbor$ in $parent\_map[from\_clique]$}
            \If{$neighbor \neq to\_clique$ and $(neighbor, from\_clique) \in messages$}
                \State $variables\_seen \gets variables\_seen \cup$ variables in $messages[(neighbor, from\_clique)]$
            \EndIf
        \EndFor
        \State Initialize $message\_to\_send$ with all entries set to 1
        \For{each possible assignment in $message\_to\_send$}
            \State Compute index in $from\_potential$
            \State Multiply with clique potential and incoming messages
        \EndFor
        \State Store message in $messages[(from\_clique, to\_clique)]$
    \EndProcedure

    \State $messages \gets \{\}$
    \State $clique\_potentials \gets$ getCliquePotentials()
    \State $max\_depth \gets$ max depth in $depth\_map$
    \For{$depth = max\_depth$ to $0$}
        \For{each clique with depth $d = depth$}
            \State Identify parent clique(s)
            \For{each parent $p$}
                \State \Call{SendMessage}{$clique, p, junction\_tree\_adj\_list, clique\_potentials, messages$}
            \EndFor
        \EndFor
    \EndFor
    \State \Call{SendMessage}{$root, \text{None}, junction\_tree\_adj\_list, clique\_potentials, messages$}
    \State Normalize probabilities and sort assignments
