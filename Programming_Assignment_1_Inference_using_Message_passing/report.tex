...
\Procedure{SendMessage}{$from\_clique, to\_clique, parent\_map, clique\_potentials, messages$}
    ...
    \For{each $i$ in range $2^{|variables\_seen|}$}
...
\For{depth from $max\_depth$ to $0$}
    \For{each $(clique, d)$ in $depth\_map$}
        \If{$d == depth$}
